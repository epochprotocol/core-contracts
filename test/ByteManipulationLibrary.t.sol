...
import "../src/helpers/ByteManipulationLibrary.sol";
...
contract ByteManipulationLibraryTest is Test {
    ...
    function testExtractingFixedSizeDynamicDataStringArray() public {
        ...
        bytes[] memory extractedData = this.extractFixedSizeDynamicArrayData(testData, 3);
    ...
    function testExtractingFixedSizeDynamicDataBytesArray() public {
        bytes[4] memory fixedSizeTestBytes = [bytes("A"), bytes("B"), bytes("C"), bytes("D")];
        bytes memory testData = abi.encode(1, "abcd", fixedSizeTestBytes, 4);
        bytes[] memory extractedData = this.extractFixedSizeDynamicArrayData(testData, 2);
        assertEq(extractedData.length, 4);
    ...
    function testExtractingDynamicSizeDynamicDataStringArray() public {
        ...
        bytes[] memory extractedData = this.extractDynamicSizeDynamicArrayData(testData, 3);
        assertEq(extractedData.length, 4);
        for (uint8 i = 0; i < extractedData.length; i++) {
    ...
    function testExtractingDynamicSizeDynamicDataBytesArray() public {
        ...
        bytes[] memory extractedData = this.extractDynamicSizeDynamicArrayData(testData, 2);
        assertEq(extractedData.length, 4);
    ...
    function testExtractingStaticDataAddressArray() public {
        ...
        bytes[] memory extractedData = this.extractStaticDataArrayData(testData, 2);
